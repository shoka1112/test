...
function drawLoop() {
  ...
  // canvasの情報を取得　この状態で　context.drawImage(tigerStanp〜　すると固まる
  // imageData = context.getImageData(0, 0, canvas.width, canvas.height);


  // context.drawImage(tigerStanp, canvas.width/4, canvas.height/4);//描画できるけどキャプチャできない
...
function displaySnapshot() {
  ...
  snapshot.onload = function () {
    snapshot.width = snapshot.width * 1;
    snapshot.height = snapshot.height * 1;
